...
// Spec uses 1-based and returns 0 for not found; we deviate intentionally (see
// PHASE1_DIFFERENCE.md).
RAP_Value RAP_index_of(RAP_Value needle, RAP_Value haystack) {
  // Default tuple search
  if (RAP_IS_PTR(haystack) &&
      RAP_PTR_VALUE(haystack)->tag == RAP_OBJECT_TAG_TUPLE) {
    for (uint32_t i = 0; i < RAP_PTR_VALUE(haystack)->tuple_val->count; i++) {
      if (RAP_BOOL_VALUE(RAP_equal(
              needle, RAP_PTR_VALUE(haystack)->tuple_val->items[i]))) {
  ...
  if (RAP_IS_PTR(haystack) &&
      RAP_PTR_VALUE(haystack)->tag == RAP_OBJECT_TAG_TEXT &&
      RAP_IS_PTR(needle) && RAP_PTR_VALUE(needle)->tag == RAP_OBJECT_TAG_TEXT) {
    struct RAP_Tuple *h = RAP_GET_TEXT_VAL(haystack);
    struct RAP_Tuple *n = RAP_GET_TEXT_VAL(needle);
    if (n->count == 0)
      return RAP_create_int_obj(0);
    if (n->count > h->count)
      return RAP_create_int_obj(-1);
  ...
  if (RAP_IS_PTR(haystack) &&
          RAP_PTR_VALUE(haystack)->tag == RAP_OBJECT_TAG_SLICE ||
      RAP_IS_PTR(needle) &&
          RAP_PTR_VALUE(needle)->tag == RAP_OBJECT_TAG_SLICE) {
    RAP_Value materialized_needle =
        RAP_materialize_slice(RAP_PTR_VALUE(needle));
    RAP_Value materialized_haystack =
        RAP_materialize_slice(RAP_PTR_VALUE(haystack));
    RAP_Value index_of =
        RAP_index_of(materialized_needle, materialized_haystack);
    if (RAP_IS_PTR(needle) &&
        RAP_PTR_VALUE(needle)->tag == RAP_OBJECT_TAG_SLICE)
      RAP_free_object(RAP_PTR_VALUE(materialized_needle));
    if (RAP_IS_PTR(haystack) &&
        RAP_PTR_VALUE(haystack)->tag == RAP_OBJECT_TAG_SLICE)
      RAP_free_object(RAP_PTR_VALUE(materialized_haystack));
    return index_of;
  }
...
RAP_Value RAP_create_slice(RAP_Value parent, int64_t from, int64_t to) {
  ...
  if (RAP_IS_PTR(parent) &&
      RAP_PTR_VALUE(parent)->tag == RAP_OBJECT_TAG_SLICE) {
  ...
  // For slices that are actually just a single item, expand so we include just
  // the item itself
  if (from == to && to < count)
    to += 1;
...
RAP_Value RAP_materialize_slice(RAP_Object *obj) {
  if (obj->tag != RAP_OBJECT_TAG_SLICE)
    return RAP_CREATE_PTR(obj);
