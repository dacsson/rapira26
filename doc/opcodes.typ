...
#table(
  ...
  [`0x04`], [`BINOP DIV`],   [—],     [Pop two integers, push integer quotient (TOS-1 ÷ TOS).],
...
The `BINOP` lower nibble encodes the operator (1–13 = `ADD`…`OR`). Opcodes `0x0e`–`0x0f` are invalid.
...
#table(
  ...
  [`0x1c`], [`CONSTF`], [`f64le value`],      [Push immediate 64-bit floating-point `value` (little-endian IEEE 754) onto the stack.],
)
...
#table(
  ...
  [`0x52`], [`BEGIN`],          [`i32le args`, `i32le locals`],  [Start of procedure with `args` arguments and `locals` local variables. Initialises locals to empty. Cannot use captured variables.],
  [`0x53`], [`CBEGIN`],         [`i32le args`, `i32le locals`],  [Start of closure body with `args` arguments and `locals` locals. May reference captured variables.],
  [`0x54`], [`CLOSURE`],        [`i32le offset`, `i32le arity`], [Construct a closure whose bytecode starts at `offset`. `arity` is the number of captured variables.],
  [`0x55`], [`CALLC`],          [`i32le arity`],                 [Call the closure on TOS-(`arity`) with `arity` arguments; push the return value.],
  [`0x56`], [`CALL`],           [`i32le offset`, `i32le n`],     [Call the function whose bytecode starts at `offset` with `n` arguments; push the return value.],
  [`0x58`], [`ARRAY`],          [`i32le n`],                     [Test whether TOS is an array of exactly `n` elements.],
  [`0x5a`], [`LINE`],           [`i32le n`],                     [Annotation: following bytecode corresponds to source line `n`. Used for diagnostics only.],
)
...
#table(
  ...
  [`0x71`], [`CALLBUILTIN Lwrite`],  [`i32le n`],   [Call built-in `Lwrite`: pop TOS and write it to stdout. `n` encodes flags (e.g. newline bit at `1 << 30`).],
  [`0x72`], [`CALLBUILTIN Llength`], [—],           [Call built-in `Llength`: pop string/array, push its length.],
  [`0x73`], [`CALLBUILTIN Lstring`], [—],           [Call built-in `Lstring`: load string from string table (string index is on the stack).],
  [`0x74`], [`CALLBUILTIN Barray`],  [`i32le n`],   [Call built-in `Barray`: construct an array from the top `n` stack values.],
  [`0x75`], [`BOOL`],                [`u8`],        [Push a boolean value onto the stack. Operand: non-zero = #true, zero = #false.],
)

= Summary Table

All #(44) defined opcode bytes at a glance.
